...
import io.github.dockndevai.ossian.config.KeycloakRoleConverter;
...
class KeycloakRoleConverterTests {
	...
	void mapsRealmRolesToPrefixedAuthorities() {
		var authorities = this.converter
			.convert(jwt(Map.of("realm_access", Map.of("roles", List.of("ossian-admin", "ossian-user")))));

		assertThat(authorities).extracting(GrantedAuthority::getAuthority)
			.contains("ROLE_ossian-admin", "ROLE_ossian-user");
	}
